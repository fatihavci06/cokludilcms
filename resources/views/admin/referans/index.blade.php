...
@section('content')
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="bgc-white bd bdrs-3 p-20 mB-20">
                    <h4 class="c-grey-900 mB-20">Referanslar</h4>
        ...
        <tbody id="sortable">
          @foreach($data as $d)
            <tr id="referans_{{$d->id}}">
                <td>{{$d->id}}</td>
                <td style="display: none;" >{{$d->order_number}}</td>
               
                <td><img src="{{Storage::url($d->image)}}" width="30px" height="30px"></td>
                
                <td><a class="btn btn-primary" href="{{route('referans.edit',$d->id)}}">Düzenle</a></td>
                <td><a  onclick="silmedenSor({{'"'.route('referans.delete',$d->id).'"'}});return false" class="edit btn btn-danger btn-sm">Sil</a></td>
            </tr>
          @endforeach
  ...
  $( function() {
    $( "#sortable" ).sortable({
        ...
        update:function(){
         ...
         $.get("{{route('referans.sortable')}}?",siralama,function(data,status){
            console.log(data);
         });
        }
    });
